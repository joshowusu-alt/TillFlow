...
export async function deleteCustomerAction(formData: FormData): Promise<void> {
  return formAction(async () => {
    ...
    const deleted = await deleteCustomer(id, businessId);
    if (!deleted) return err('Customer not found. It may have already been removed.');

    audit({ businessId, userId: user.id, userName: user.name, userRole: user.role, action: 'CUSTOMER_DELETE', entity: 'Customer', entityId: id, details: { name: deleted.name } }).catch(() => {});
