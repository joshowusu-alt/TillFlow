...
describe('TillFlow brand logo system', () => {
  ...
  it('defers protected shell badge data so launch content is not blocked by nav KPIs', () => {
    ...
    expect(topNav).toContain('Do not fetch on first paint');
  ...
  it('uses React-rendered root loading instead of a pre-hydration DOM splash', () => {
    const layout = readSource('app/layout.tsx');
    const rootLaunchLoading = readSource('components/RootLaunchLoading.tsx');
