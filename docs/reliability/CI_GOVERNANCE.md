...
- `lib/database-target-guard.ts` (no Prisma import) decides whether a URL may receive test writes. Deny rules win: database `neondb` (taken from the **first** path segment, exactly as Prisma does, so `…/neondb/` cannot evade it), fragment `fancy-darkness` anywhere in the credential-stripped URL (covers Neon `options=endpoint=…` routing, not just the hostname), `VERCEL_ENV=production`, anything in `TILLFLOW_PRODUCTION_DB_HOSTS`. Allow: local hosts; a **known isolated Neon endpoint** (`old-sunset`, `late-cell` — `ISOLATED_PREVIEW_ENDPOINT_FRAGMENTS`) **combined with** a database name matching `tillflow_(ci|preview|test|walkthrough|qa)*`; or an explicit `TILLFLOW_TEST_DB_ALLOWLIST` (`host/db,…`) pair. A database merely *named* `tillflow_preview` on an unknown remote endpoint is refused — a new Neon compute id (including a future Production one) is closed by default. Everything else is refused. Missing URLs are refused. Contradictory URLs (e.g. `DATABASE_URL` = Preview while `POSTGRES_PRISMA_URL` = Production; a different port counts as a different target) are refused unless `TILLFLOW_TEST_DATABASE_URL` names the target explicitly. `isolated-postgres.ts` and `final-walkthrough-gate.pg.test.ts` share the same endpoint list via `isIsolatedPreviewIdentity()` (the earlier `old-sunset`-only check pre-dated the `late-cell` branch and had drifted).
