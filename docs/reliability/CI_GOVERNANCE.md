...
- `lib/database-target-guard.ts` (no Prisma import) decides whether a URL may receive test writes. Deny rules win: database `neondb` (taken from the **first** path segment, exactly as Prisma does, so `…/neondb/` cannot evade it), fragment `fancy-darkness` anywhere in the credential-stripped URL (host + path + query decoded **per pair** via `URLSearchParams`, plus the raw query — so Neon `options=endpoint=…` routing is covered and a malformed `%zz` elsewhere cannot switch decoding off; `sslpassword`/`password`/`passfile`/`sslkey` values are dropped — keys compared after decoding — so the text is safe to attach to errors; the decoded userinfo is deny-matched as well, because Neon's SNI-less workaround puts `endpoint=ep-…` in the password, but it is never stored or logged), `VERCEL_ENV=production`, anything in `TILLFLOW_PRODUCTION_DB_HOSTS`. Allow: local hosts; a **known isolated Neon endpoint** (first host label `ep-(old-sunset|late-cell)-<id>[-pooler]` **and** host ending `.neon.tech` — `knownIsolatedEndpointFragment()`; `late-cell.example.com` and `ep-late-cell-1.attacker.net` do not qualify) **combined with** a database name matching `tillflow_(ci|preview|test|walkthrough|qa)*`; or an explicit `TILLFLOW_TEST_DB_ALLOWLIST` (`host/db,…`) pair. A database merely *named* `tillflow_preview` on an unknown remote endpoint is refused — a new Neon compute id (including a future Production one) is closed by default. Everything else is refused. Missing URLs are refused. Contradictory URLs (e.g. `DATABASE_URL` = Preview while `POSTGRES_PRISMA_URL` = Production; a different port counts as a different target) are refused unless `TILLFLOW_TEST_DATABASE_URL` names the target explicitly. `isolated-postgres.ts` and `final-walkthrough-gate.pg.test.ts` share the same endpoint list via `isIsolatedPreviewIdentity()` (the earlier `old-sunset`-only check pre-dated the `late-cell` branch and had drifted).
...
- `lib/test/test-prisma.ts` is the **only** way a test obtains a client: `openTestPrismaClient()` guards, pins, resets the app singleton, constructs with an explicit datasource, connects and proves `current_database()` before returning. `runTestTeardown()` runs every cleanup step, disconnects, then throws an aggregate — teardown can no longer swallow a failed delete; all 18 live-database suites use it (the `.catch(() => {})` form is gone, and `final-walkthrough-gate.pg.test.ts`, which previously never deleted its seeded tenant, now does). If `$connect()` or the identity proof fails, the factory disconnects the half-built client before rethrowing. `lib/test/isolated-postgres.ts` is a thin wrapper (`openBoundPrismaClient`); its old `POSTGRES_URL_NON_POOLING`-first precedence and synchronous `createBoundPrismaClient` are gone.
- `lib/reliability/database-test-safety.test.ts` (unit job, `npm run guard:db`) fails the build if any test file — or any non-test helper under `lib/test/` or `tests/` — constructs a Prisma client (`new PrismaClient(`, `new P.PrismaClient(`, `new (PrismaClient)(`), imports or re-exports `PrismaClient` as a runtime value from `@prisma/client`, any sub-path, `.prisma/client` or a relative path into it (named, namespace, default, `require`, dynamic `import()`, `vi.importActual()`, `export … from`, template-literal specifiers; `type` imports are fine) — helpers are checked for value imports too, so a helper cannot launder the class to tests under another name — or assigns Prisma URL variables (`=`, `??=`, `||=`, `+=`, bracket form, `Object.assign/defineProperty/defineProperties(process.env, …)`, `Reflect.set(process.env, …)`, `vi.stubEnv('X', …)`, `delete process.env.X`); the scan self-tests its own regexes against these evasions. It also freezes the list of pre-existing bare-client scripts under `scripts/` and `prisma/` (an entry that stops constructing a bare client must be removed) — new scripts must use `scripts/lib/guarded-prisma.cjs` (same rules; Production is openable only with `{ readOnly: true }` + `TILLFLOW_ALLOW_PRODUCTION_READ=1`, and the session is forced `READ ONLY`). Outside the gate, by design: `scripts/*-pg-test.cjs` use the raw `pg` driver (which does not load `.env`, so the incident mechanism does not apply) and read `POSTGRES_URL_NON_POOLING` from the workflow env; scripts that import the `lib/prisma` singleton inherit its guard only under `VITEST`.
