...
Status: formula and balance amendment applied (2026-09-23). Not implemented. No current report route is deleted by this document.
...
| S46 | Customer statement | `GET /customers/[id]/statement` | export | invoice rows | `customer_receivables` | delivery | Own-records statement. Paid and Balance must match the A12 receivables helper before confidence can rise | none | All plans | M+O | M+O | LOW | Field split: invoice identity, date, status, and stored total are a direct extract with no reconciliation fixture (MEDIUM). Paid and Balance sum every `payments.amountPence` with no payment-status filter (`statement/route.ts`), unless the invoice is `RETURNED` or `VOID`. Overall LOW until Paid and Balance reconcile to the canonical receivables helper | Wave A | Wave A |
| S47 | Supplier statement | `GET /suppliers/[id]/statement` | export | purchase rows | `supplier_payables` | delivery | Own-records statement. Paid and Balance must match the A12 payables helper before confidence can rise | none | All plans for the rows. Supplier-debt fields need the grant | M+O | `VIEW_SUPPLIER_DEBT` | LOW | Field split: purchase identity, date, status, and stored total are a direct extract with no reconciliation fixture (MEDIUM). Paid and Balance sum every purchase payment. `PurchasePayment` has no status column, so the sum is not an authoritative status filter (`suppliers/[id]/statement/route.ts`). Overall LOW until Paid and Balance reconcile to the canonical payables helper. The grant is a separate Wave C change | Wave A for the balance helper. Wave C for the grant | Wave A then Wave C |
