...
export default function StatCard({ label, value, tone = 'default', helper }: StatCardProps) {
  return (
    <div
      className={`relative overflow-hidden rounded-[1.25rem] border p-4 transition-all duration-200 sm:rounded-[1.4rem] sm:p-5 ${tones[tone]}`}
    ...
    >
      ...
      <div className="relative">
        ...
        <div className={`mt-3 text-[1.65rem] font-display font-bold tabular-nums tracking-tight sm:text-2xl md:text-[2rem] ${valueColors[tone]}`}>{value}</div>
